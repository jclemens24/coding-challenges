...
export const gap = (g: number, m: number, n: number): number[] | null => {
  ...
  let prev: number = 0;
  // Use Bitwise OR to make sure a bit is on
  // Essentially, we don't want even numbers here. So if an even number is passed, it
  // It will be | 1 to turn on that last bit, making it odd
  for (let curr = m | 1; curr <= n; curr += 2) {
    console.log('curr: %f', curr);
