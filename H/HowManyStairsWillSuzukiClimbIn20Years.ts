...
export function stairsIn20(stairs: number[][]) {
  return (
    stairs.reduce((acc, curr, i) => {
      if (Array.isArray(curr)) {
        return acc + curr.reduce((prev, next) => prev + next, 0);
      }
      return 0;
    }, 0) * 20
  );
...
console.log(
  stairsIn20([sunday, monday, tuesday, wednesday, thursday, friday, saturday])
);

export function stairsIn20V2(stairs: number[][]): number {
  const merged = ([] as number[]).concat(...stairs);
  return merged.reduce((acc, curr) => acc + curr, 0) * 20;
}
